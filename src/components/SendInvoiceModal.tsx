...
export default function SendInvoiceModal({
  ...
  const [frozenInvoiceNum, setFrozenInvoiceNum] = useState(() => generateInvoiceNumber(settings.prefix, settings.nextNumber));
  ...
  useEffect(() => {
    if (isOpen && !hasAutoSaved) {
      const currentInvoiceNum = generateInvoiceNumber(settings.prefix, settings.nextNumber);
      setFrozenInvoiceNum(currentInvoiceNum);
      
      setToEmail(client.email || '');
      setCcEmail(client.ccEmail || '');
      setBccEmail(client.bccEmail || '');
      setSubject(`Factuur ${currentInvoiceNum} - ${company.name}`);
      
      const formattedTotal = formatCurrency(totals.total);
      const defaultBody = `Beste ${client.name || 'Klant'},\n\nHierbij ontvangt u factuur ${currentInvoiceNum} voor onze recent geleverde diensten/producten.\n\nDetails:\n- Factuurnummer: ${currentInvoiceNum}\n- Totaalbedrag: ${formattedTotal}\n- Vervaldatum: ${settings.dueDate}\n\nWij verzoeken u vriendelijk om het bedrag over te maken naar IBAN ${company.iban} ten name van ${company.name}, onder vermelding van het factuurnummer ${currentInvoiceNum}.\n\nIn de bijlage vindt u de volledige specificatie als PDF.\n\nMet vriendelijke groet,\n\n${company.name}\n${company.email || ''}`;
      
      setMessage(defaultBody);
    }
  }, [isOpen, client, company, settings.dueDate, settings.prefix, settings.nextNumber, totals.total, hasAutoSaved]);
  ...
  const handleDownloadPdf = () => {
    ...
    document.title = frozenInvoiceNum;
    window.print();
  ...
  const handleMailto = () => {
    ...
    onSuccess(`Uw e-mailprogramma is geopend! Vergeet niet de gedownloade PDF (${frozenInvoiceNum}.pdf) toe te voegen als bijlage.`);
    onClose();
  };
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4 text-slate-300">
          ...
          <div className="bg-slate-950 p-3 rounded-xl border border-slate-850 flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <p className="text-xs text-slate-200 font-semibold">{frozenInvoiceNum}.pdf</p>
